...
ERROR_INDICATORS: list[tuple[str, str, Severity]] = [
    ...
    (r"Traceback \(most recent call last\)",
     "Python stack trace", Severity.HIGH),
    (r"at\s+[\w\.]+\([\w\.]+\.(?:java|kt):\d+\)",
     "Java/Kotlin stack trace", Severity.HIGH),
    (r"System\.Web\.HttpException",
     "ASP.NET exception class", Severity.HIGH),
    (r"Microsoft\.CSharp\.|System\.Web\.",
     ".NET namespace in error response", Severity.MEDIUM),
    # Framework version disclosure
    (r"Django[\s/][\d\.]+",
     "Django version disclosure", Severity.MEDIUM),
    (r"Laravel[\s/][\d\.]+",
     "Laravel version disclosure", Severity.MEDIUM),
    (r"Ruby on Rails[\s/][\d\.]+",
     "Rails version disclosure", Severity.MEDIUM),
    (r"Express[\s/][\d\.]+",
     "Express.js version disclosure", Severity.MEDIUM),
    (r"Flask[\s/][\d\.]+",
     "Flask version disclosure", Severity.MEDIUM),
    (r"Spring[\s/][\d\.]+",
     "Spring version disclosure", Severity.MEDIUM),
    # Explicit debug-mode markers
    (r"DEBUG\s*=\s*True",
     "Django DEBUG=True in response", Severity.HIGH),
    (r"SQLSTATE\[",
     "Raw SQL error / SQLSTATE code", Severity.HIGH),
    (r"ORA-\d{5}",
     "Oracle database error code", Severity.HIGH),
    (r"com\.mysql\.jdbc\.",
     "MySQL JDBC driver class in response", Severity.HIGH),
    (r"Caused by:\s+\w",
     "Java 'Caused by' exception chain", Severity.HIGH),
    # Internal paths
    (r"/home/\w+/|/var/www/|/srv/",
     "Internal filesystem path disclosed", Severity.MEDIUM),
    (r"C:\\(?:Users|inetpub|Windows)\\",
     "Windows filesystem path disclosed", Severity.MEDIUM),
]
...
class InfoDisclosureScanner(BaseScanner):
    ...
    description = (
        ...
        "(error pages, debug endpoints, comments)"
    )
    ...
    def scan(self, target: Target) -> ScannerResult:
        ...
        if self.options["check_debug_endpoints"]:
            result.findings.extend(
                self._check_debug_endpoints(session, target)
            )

        if self.options["check_html_comments"]:
            result.findings.extend(
                self._check_html_comments(session, target)
            )

        if self.options["check_version_endpoints"]:
            result.findings.extend(
                self._check_version_endpoints(session, target)
            )
