...
_SCANNER_INFO = [
    {"name": "Nuclei", "description": "Template-based CVE, misconfiguration, and exposure scanner", "version": "3.x", "icon": "radar"},
    {"name": "Nikto", "description": "Web server misconfiguration and outdated software scanner", "version": "2.x", "icon": "search"},
    {"name": "Wapiti", "description": "Black-box web application vulnerability scanner", "version": "3.x", "icon": "shield"},
    {"name": "Nmap", "description": "Network reconnaissance and port/service scanner", "version": "7.x", "icon": "network"},
    {"name": "SSLyze", "description": "TLS/SSL configuration and certificate analyser", "version": "6.x", "icon": "lock"},
]
...
async def scans_page(
        ...
        memory_scans = [
            {
                ...
                "risk_posture": "",
                "finding_count": len(s["session"].all_findings) if s.get("session") else 0,
            }
    ...
    return templates.TemplateResponse("scans.html", {
        ...
        "total": total,
        "hostnames": hostnames,
        "filters": {"hostname": hostname, "severity": severity,
                     "date_from": date_from, "date_to": date_to},
...
async def start_scan(request: Request) -> JSONResponse:
    ...
    report_formats = form.getlist("report_formats") or ["markdown", "json"]

    excluded_paths_raw = str(form.get("excluded_paths", "")).strip()
    excluded_paths = [p.strip() for p in excluded_paths_raw.split("\n") if p.strip()]

    rate_limit = float(form.get("rate_limit", 10.0))
    max_requests = int(form.get("max_requests", 10000))
...
async def list_scans(
    ...
    if format == "html":
        ...
        html = templates.get_template("_scan_rows.html").render(
            scans=rows, pagination=pagination,
        )
...
async def get_enrichment(scan_id: str) -> JSONResponse:
    ...
    if scan["status"] != ScanStatus.COMPLETED or not session:
        raise HTTPException(
            status_code=404, detail="Scan not found or not complete"
        )
...
def _evict_expired_conversations() -> None:
    ...
    while len(_conversations) > _CONVERSATION_MAX_COUNT:
        oldest = min(
            _conversations, key=lambda k: _conversations[k].created_at
        )
...
async def get_conversation(
    scan_id: str, conversation_id: str
) -> JSONResponse:
